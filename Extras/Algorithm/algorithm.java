...
public class algorithm {
    ...
    public static int xnum =10;//the number of surveys
    public static int ynum =10;//the number of questions
    ...
    public static void main(String args[]){
        ...
        System.out.println(onestring(findneigh(midpoint(surveys[pairs[k].x],surveys[pairs[k].y]),surveys)));
        }
        System.out.println();
    ...
    // prints a two dimensional int array
    ...
    //prints a two dimensional char array
    ...
    //checks if two points are within range of eachother
    ...
    public static int[] findneigh(int[] x, int[][] y)
    {
        int tempcnt = 0;
        int[] neighnum = new int[xnum];
        for(int j=0;j<neighnum.length;j++){
            neighnum[j] = -1;
        }
        Stack<Integer> s =new Stack<Integer>() ;
        double[] tempdistarr = new double[xnum];
        for(int i = 0; i<y.length;i++){
            tempdistarr[i]= distance(x,y[i]);
            if(tempdistarr[i]<(avgdist*.6)){
                s.push(i);
            }
        }
        while (!(s.isEmpty())) {
        neighnum[tempcnt]=s.pop();
        tempcnt++;
        }
    ...
    public static int[] midpoint(int[]x,int[]y){
        int[] midpnt = new int[xnum];
        for(int i =0; i <midpnt.length;i++){
            midpnt[i] = (x[i]+y[i])/2;
        }
        return midpnt;
    }
